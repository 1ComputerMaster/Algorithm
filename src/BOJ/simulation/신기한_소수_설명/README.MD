...
- 간단하게 부분 집합의 선택하고 안 하고를 반복 하는 것으로 생각하고 구현하였으나 다음번에 체킹 할 때 배열을 새로 초기화 해야 하므로 엄청나게 많은 횟수를 돌거나 반복적으로 갔던 곳을 또 갈 위험이
  있었습니다. (한 번 앞에서 선택 한 것을 다시 선택 할 수 없었음 -> 부분집합 문제 처럼 풀 수 없음)
